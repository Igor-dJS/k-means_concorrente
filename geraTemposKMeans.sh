...
quantidadeThreads=("1" "2" "4" "8")    # Quantidades de threads a serem testadas
n_clusters=3
...
# Execução do K-Means concorrente com diferentes números de threads
for threads in "${quantidadeThreads[@]}"; do
    for pontos in "${quantidadePontos[@]}"; do
        for i in $(seq 1 $quantidadeExecucoes); do
            echo "Execução Concorrente - Dataset com $pontos pontos e $threads threads (Execução $i)..."
            python k_means_concorrente_main.py "./datasets/dataset_${pontos}.csv" $n_clusters $threads "./logs/log_execucao_kmeans.csv"
        done
    done
done
